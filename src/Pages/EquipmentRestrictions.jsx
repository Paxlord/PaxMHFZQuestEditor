...
import {
  ReadEquipmentRestrictions,
  WriteEquipments,
} from "../Utils/QuestParams/equipment_utils";
...
const EquipmentRestriction = () => {
  ...
  const [equipments, setEquipments] = useImmer(() =>
    ReadEquipmentRestrictions(questDataView)
  );
    ...
    <div className="p-4 flex flex-col gap-y-3 ">
      <Panel onSave={() => onSave()}>
        ...
        {Object.keys(equipments).map((key) => {
          ...
          return (
            <div className="my-8">
              <h1 className="text-lg capitalize font-medium text-green-500 mb-1">
                {key}
              </h1>
              <div className="flex gap-x-3">
                <NumeralInput
                  label={"Piece ID"}
                  defaultValue={equipments[key].piece_id}
                  onChange={(value) => updateEquipments(key, "piece_id", value)}
                />
                <NumeralInput
                  label={"Deco 1"}
                  defaultValue={equipments[key].deco_1}
                  onChange={(value) => updateEquipments(key, "deco_1", value)}
                />
                <NumeralInput
                  label={"Deco 2"}
                  defaultValue={equipments[key].deco_2}
                  onChange={(value) => updateEquipments(key, "deco_2", value)}
                />
                <NumeralInput
                  label={"Deco 3"}
                  defaultValue={equipments[key].deco_3}
                  onChange={(value) => updateEquipments(key, "deco_3", value)}
                />
              </div>
            </div>
          );
