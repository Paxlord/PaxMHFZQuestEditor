...
import {
  ...
  ReadAllRewards,
  WriteRewards,
} from "../Utils/QuestParams/rewards_utils";
...
const QuestRewards = () => {
  ...
  const [rewardFlag, setRewardFlag] = useState(() =>
    ReadRewardVariant(questDataView)
  );
  ...
  const deleteReward = (indexItem, indexBox) => {
    setRewards((draft) => {
      draft[indexBox].rewards = draft[indexBox].rewards.filter(
        (item, idx) => idx !== indexItem
      );
  ...
  const addReward = (indexBox) => {
    if (
      rewards[indexBox].rewards.length >=
      rewards[indexBox].rewardHeader.rewardMaxSlot
    ) {
    ...
    <div className="p-4 flex flex-col gap-y-3 ">
      <Panel onSave={() => SaveFlag()}>
        ...
        <div className="flex gap-x-3 items-center">
          <SelectComponent
            title={"Reward Flag"}
            defaultValue={rewardFlag}
            options={RewardOptions}
            onChange={(value) => setRewardFlag(parseInt(value))}
          />
        </div>
      </Panel>
      ...
      <NoButtonPanel>
        <div className="flex items-center gap-x-3">
          <BasicButton onClick={() => addBox()} disabled={rewards.length >= 5}>
            Add Box {rewards.length + 1}
          </BasicButton>
          <BasicButton onClick={() => SaveRewards()} disabled={!boxWasRemoved}>
            Save Removed Boxes
          </BasicButton>
        </div>
      </NoButtonPanel>
      ...
      {rewards.map((reward, boxIdx) => {
        return (
          <Panel onSave={() => SaveRewards()}>
            <PanelTitle
              title={`Box number ${reward.rewardHeader.rewardBoxId}`}
            />
            <div className="flex gap-x-3 items-center">
              <button
                onClick={() => addReward(boxIdx)}
              ...
              <p
                className={`text-lg ${
                  reward.rewards.reduce(
                    (sum, rewardItem) => (sum += rewardItem.percent_chance),
                    0
                  ) === 100
                    ? "text-green-400"
                    : "text-red-400"
              ...
              {reward.rewards.map((rewardItem, itemIdx) => {
                return (
                  <div className="flex justify-evenly items-center">
                    <NumeralInput
                      label={"Percentage Chance"}
                      defaultValue={rewardItem.percent_chance}
                      onChange={(value) =>
                        updateReward("percent_chance", value, itemIdx, boxIdx)
                      }
                    />
                    <NumeralInput
                      label={"Item ID"}
                      defaultValue={rewardItem.item_id}
                      onChange={(value) =>
                        updateReward("item_id", value, itemIdx, boxIdx)
                      }
                    />
                    <p className="w-36 translate-y-3 text-white">
                      {rewardItem.item_id > 0 &&
                        rewardItem.item_id <= Items.length &&
                        Items[rewardItem.item_id]}
                    </p>
                    <NumeralInput
                      label={"Item Count"}
                      defaultValue={rewardItem.item_count}
                      onChange={(value) =>
                        updateReward("item_count", value, itemIdx, boxIdx)
                      }
                    />
                    <button
                      ...
                      onClick={() => deleteReward(itemIdx, boxIdx)}
                    >
                      Delete
                    </button>
                  </div>
      ...
      <Panel>
        ...
        <div className="flex gap-x-3 flex-wrap items-center">
          <TextArea
            defaultValue={JSON.stringify(rewards)}
            label="export rewards"
          />
          <TextArea
            label="import data"
            onChange={(value) => setRewardImportStr(value)}
          />
          <button
            className="transition h-8 px-4 py-1 hover:shadow-md bg-green-500 shadow-sm rounded text-white hover:bg-green-600 active:bg-green-700"
            onClick={() => setRewards(JSON.parse(rewardImportStr))}
          >
            Import data
          </button>
          <TextArea
            defaultValue={beautifulRewards()}
            label="Beautiful Rewards"
          />
        </div>
      </Panel>
    </div>
