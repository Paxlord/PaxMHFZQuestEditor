...
import {
  ReadQuestStrings,
  WriteStrings,
} from "../Utils/QuestParams/string_utils";
import encartsvg from "../assets/mhfencart.svg";

import {
  Objectives,
  ObjectiveCategory,
  ObjectiveNumCategory,
...
const QuestStrings = () => {
  ...
  const [questStrings, setQuestStrings] = useImmer(() =>
    ReadQuestStrings(questDataView)
  );
  ...
  const fillSuccessFailure = () => {
    ...
    let failureString = questflags[3].flag
      ? `Faint ${deathCount} Times\n`
      : `Rewards falls to 0z\n`;
      ...
      <Panel onSave={() => SaveStrings()}>
        ...
        <div className="mb-6 flex gap-x-3">
          <button
            ...
            className="transition px-4 py-1 hover:shadow-md bg-green-500 shadow-sm rounded text-white hover:bg-green-600 active:bg-green-700"
          >
            Auto-fill Objective
          </button>
          <button
        ...
        <div className="gap-y-4 flex flex-col">
          {questStrings.map((string, idx) => {
            return (
              <TextArea
                label={StringsLabel[idx].label}
                rows={StringsLabel[idx].rows}
                defaultValue={string.string}
                chara_count={true}
                onChange={(value) => updateString(idx, value)}
              />
            );
          })}
        </div>
