...
export const ReadAllRewards = (dataview) => {
  ...
  rewardBoxesInfo.forEach((rewardBox) => {
    let rewardList = ReadBox(
      dataview,
      rewardBox.rewardBoxAddr,
      rewardBox.rewardMaxSlot
    );
    rewardBoxes.push({
...
export const WriteRewards = (dataview, rewards) => {
  ...
  if (
    questRewardsPointer < stringsSectionPointer &&
    newQuestRewardsAddr >= dv.byteLength
  ) {
  ...
  for (let i = 0; i < rewards.length + 1; i++) {
    ...
    dv = immu_write_ushort(
      dv,
      currentItemOffSet + 3,
      rewardHeader.rewardBoxUnk2
    );

    const currentBoxAddr =
      newQuestRewardsAddr + (rewards.length + 1) * 8 + i * newBoxSectionSize;
